...
public interface IArticuloService {
	    ...
	    @Query(value = "select * from vw_articulosBR  where  lista=?1",nativeQuery = true)
	    ...
	    @Query(value = "select V from vw_articulosBR V where V.importe <=?1 and V.lista=?2")
	    Page<vw_articulosBR_row> findAllByListaAndImporte(double importe , String lista, Pageable pegeable);

	    @Query(value = "select V from vw_articulosBR V where V.fabricante =?1 and V.lista=?2",nativeQuery = true)
	    ...
	    @Query("select categoria from vw_articulosBR where lista =?1  GROUP BY categoria ")
	    List<String> findByCategoriaL(String lista,String categoria);
	    @Query("select grupo from vw_articulosBR where categoria =?1 and lista =?2 GROUP BY grupo")
	    List<String> findByGrupo(String categoria,String lista);
	    @Query("select familia from vw_articulosBR  where categoria =?1 and grupo =?2 and lista =?3  GROUP BY familia")
